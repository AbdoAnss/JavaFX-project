...
public class loginView extends Application{
	...
	public void start(Stage stage) throws Exception {
		...
		RadioButton rdBtn1 = new RadioButton("Manager");
				...
				if (rdBtn1.isSelected()) {
					for (int i=0; i<list.size(); i++) {
						...
						if (tab[7].equals(user) && tab[8].equals(passwd) && tab[9].equals("isManager")) {
							...
							loginMessage.setText("Bonjour cher Manager!");
							stage.close();
					...
					for (int i=0; i<list.size(); i++) {
						...
						if (tab[7].equals(user) && tab[8].equals(passwd) && tab[9].equals("isIntervenant")) {
							Session.setutilisateurConnecte(tab);
							loginMessage.setFill(Color.GREEN);
							loginMessage.setText("Bonjour cher Intervenant!");
							stage.close();
							mainView main = new mainView();
							main.start(new Stage());
							break;
					...
					loginMessage.setText("Mot de passe ou Identifiant est invalide!");
		...
		vbox.getChildren().addAll(imageview,loginMessage,tf1,tf2,hbox,btn);
		vbox.setSpacing(10);
