...
public class mainView extends Application {
	...
	private SimpleStringProperty matCountProperty = new SimpleStringProperty("Vous avez 3 matreiels");
	...
	public void start(Stage stage) throws Exception {
		...
		ObservableList<Tache> list = FXCollections.observableArrayList(
					new Tache(81456293,"tache 1"," description du projet 1",projet),
					new Tache(95138742,"tache 2","description du projet 2",projet),
					new Tache(76329483,"tache 3"," description du projet 3",projet)
				);
		...
		Label nbrMateriel = new Label();
		nbrMateriel.textProperty().bind(matCountProperty);
		Button ajouterMateriel = new Button(" Ajouter un matériel ");
		
		ajouterMateriel.setOnMouseClicked((evt)->{
			AddMateriel addMat = new AddMateriel();
			try { addMat.start(new Stage()); }
			catch (Exception e) {e.printStackTrace(); }
			Materiel newMat = addMat.getNewMateriel();
			materiels.add(newMat);
			matCountProperty.set("Vous avez " + materiels.size() + " matériaux");
		});
